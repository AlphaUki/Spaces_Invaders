...
def load_images(file_format, nb):
...
class Font:
    ...
    @staticmethod
    def text_as_image(text, color = None):
...
def load_sounds(file_format, nb):
...
class Bullet:
    def __init__(self, canvas, defender):
        self.canvas = canvas
...
class Alien:
    def __init__(self, canvas, x, y, frames, tag, worth):
        self.canvas = canvas
        self.start_pos = x, y
...
class Bomb:
    def __init__(self, canvas, fleet, alien):
        self.canvas = canvas
        self.fleet = fleet
        self.alien = alien
        self.current_frame = 0
